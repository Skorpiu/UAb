...
INPUT:  venda_validar_artigo_cliente('David', 'Rato Otico', 26).
*/
venda_validar_artigo_cliente(Nome, Artigo, Quantidade_a_comprar) :-
    ...
    format('Referencia: ~w ~nCliente: ~w ~nFinalizar Venda!', [Referencia, Nome]), !, true;
    format('Referencia: ~w ~nCliente: ~w ~nFalha!', [Referencia, Nome]), !, false.
...
venda_artigo_cliente :-
    ...
    format('~nCliente: ~w~nArtigo: ~w~nQuantidade: ~w~n~n', [Nome, Artigo, Quantidade_a_comprar]),
    (clientes(Nome,_,'aaa'),
    artigo(Referencia, Artigo,_),
    inventario(Referencia, Quantidade),
    Quantidade_a_comprar =< Quantidade,
    ...
    write('Venda concluida com sucesso!')),
    !, true; 
    nl, write('Falha!'), !, false.

/*
INPUT:  inventario_relatorio.
*/
inventario_relatorio :-
    listing(inventario).

/*
------------------ Funções auxiliares de debuggung -------------------
